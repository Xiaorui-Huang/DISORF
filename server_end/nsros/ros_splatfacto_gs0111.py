...
class SplatfactoModel(Model):
    ...
    def populate_modules(self):
        if self.seed_points is not None and not self.config.random_init:
            ...
            if not self.config.uncontract_point_init:
                ...
                pts_c = (torch.rand((self.config.num_random, 3)) - 0.5) * 4.0 # [-2, 2]
                pts = uncontract(pts_c) * self.config.random_scale * 0.25
    ...
    def get_outputs(self, camera: Cameras) -> Dict[str, Union[torch.Tensor, List]]:
        ...
        if self.training or self.use_training_cameras:
            ...
            if p_step and optimized_camera_to_world.requires_grad:
                pass
        else:
        ...
        if self.crop_box is not None and not self.training:
            ...
            crop_ids = None
        camera_downscale = self._get_downscale_factor(step=p_step)  # scale scheduling!!
        ...
        return {"rgb": rgb, "depth": depth_im, "accumulation": alpha, "background": background, "p_step": p_step}  # type: ignore
